...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        if (product && products.length > 0) {
            ...
            const sameBrandProducts = products.filter((p) => p.brand_id === product.brand_id && p.id !== product.id).slice(0, 4); // Limit to 4 related products
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    ${selectedImage === img ? "ring-2 ring-primary" : ""}`}
                                ...
                                >
                                    <Image src={img} alt={`${product.name} - view ${index + 1}`} fill className="object-contain p-1" />
                                </div>
                            ))
                        ) : (
                            <div className="aspect-square relative rounded-md overflow-hidden border">
                                <Image src="/laptop.png" alt={product.name} fill className="object-contain p-1" />
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col space-y-4 mt-6">
                        ...
                        <div className="flex items-center">
                            ...
                            <span className="ml-4 text-sm text-muted-foreground">{product.stock} sản phẩm có sẵn</span>
                        </div>

                        {/* Add to cart button */}
                        <Button className="w-full" size="lg" onClick={handleAddToCart} disabled={isAddingToCart || product.stock <= 0}>
                            {isAddingToCart ? (
                                <>Đang thêm...</>
                            ) : (
                                <>
                                        ...
                                        <div className="overflow-hidden rounded-lg border shadow-sm">
                                            <table className="w-full border-collapse">
                                                ...
                                                <tbody>
                                                    ...
                                                    {Array.from({
                                                        length: Math.ceil(product.details.specifications.length / 2),
                                                    }).map((_, rowIndex) => {
                                                        // Lấy 2 thông số cho mỗi hàng
                                                        const startIdx = rowIndex * 2;
                                                        return (
                                                            <tr key={rowIndex} className={`${rowIndex % 2 === 0 ? "bg-blue-50" : "bg-gray-100"}`}>
                                ...
                                <Link href={`/product/${relatedProduct.id}`} className="block">
                                    ...
                                    <CardContent className="p-4">
                                        <div className="flex flex-col h-full">
                                            ...
                                            <div className="mt-2 font-bold text-primary text-lg">{formatCurrency(relatedProduct.price)}</div>
